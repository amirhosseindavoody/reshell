...
| File | Responsibility |
|------|----------------|
| [`src/main.rs`](../../src/main.rs) | Clap CLI: `new` / `attach` / `list` / `info` / `context` / `rename` / `clean` / `kill` / `completion`; dynamic session-name completion; detach-key + log + scrollback flags; default shell `/bin/zsh` |
